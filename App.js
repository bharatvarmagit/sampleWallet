...
export default function App() {
  ...
  const createWalletWithMnemonic=async()=>{
    const newWallet = ethers.Wallet.fromMnemonic(randomMnemonic).connect(provider)
    setWallet(newWallet)
  ...
  return (
    <View style={styles.container}>
     ...
     />
      ...
      <Button onPress={createWallet} title='create new wallet random mnemonic'/>


      <Button onPress={()=>{
        setRandomMnemonic(bip39.generateMnemonic(wordlist))
        setWallet()
        }} title={'create new wallet with chosen mnemonic'}/>
      {randomMnemonic && !wallet?
      <>
        <Text selectable> {randomMnemonic}</Text>
        <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
        <Button title="change phrase" onPress={()=>setRandomMnemonic(bip39.generateMnemonic(wordlist))}/>
        <Button title="select phrase" onPress={createWalletWithMnemonic}/>

        </View>
      </>
      :

      <Button onPress={()=>setRecoverEnable(true)} title='Import wallet' />
}
